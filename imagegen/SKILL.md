...
When the user asks for `-h` or `--help`, explain the skill like a CLI help page:

- Purpose: generate or edit raster images for project work, using the built-in `image_gen` tool by default.
- Modes:
  - `default`: built-in `image_gen` flow for normal generation, editing, and simple transparent-background requests.
  - `cli-fallback`: `scripts/image_gen.py` for explicit CLI/API workflows or confirmed true-native transparency with `gpt-image-1.5`.
- Output formats:
  - Built-in mode: generated images saved under `$CODEX_HOME/generated_images/...` unless moved into the project.
  - CLI mode: final artifacts saved under `output/imagegen/`; temporary files under `tmp/imagegen/`.
  - Common deliverables: `png`, `webp`, `jpg/jpeg`, and project-bound copied assets.
- Common examples:
  - Generate a landing-page hero image.
  - Edit an attached image to remove an object.
  - Create a transparent PNG cutout using built-in chroma-key removal first.
  - Batch-produce multiple related assets only when the user explicitly requests CLI mode.
- Safety rules:
  - Prefer built-in `image_gen`; do not silently downgrade to CLI or a different model path.
  - Ask before using true-native transparency via CLI `gpt-image-1.5`.
  - Never modify `scripts/image_gen.py`.
  - Never leave a project-referenced final asset only under `$CODEX_HOME/*`; move or copy it into the workspace.
